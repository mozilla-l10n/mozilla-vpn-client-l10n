...
def main():
    ...
    parser.add_argument(
        '--reference',  required=True, dest='reference_locale',
        help='Reference locale code')
    parser.add_argument(
        '--path', required=True, dest='base_folder',
        help='Path to folder including subfolders for all locales')
    parser.add_argument(
        '--xliff', required=True, dest='xliff_filename',
        help='Name of the XLIFF file to process')
    parser.add_argument(
        'locales', nargs='*', help='Locales to process')
    ...
    for file_path in file_paths:
        ...
        for trans_node in reference_root_copy.xpath('//x:trans-unit', namespaces=NS):
            ...
            if translated and not updated:
                ...
                xml_space = trans_node.xpath(
                    './x:source', namespaces=NS)[0].get(attrib_name)
